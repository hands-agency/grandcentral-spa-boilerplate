...
//	Here we go!
		...
		//	We can upload
			if (vars.tests.filereader === true)
			{
				...
				//	Upload!
					...
					//	Add file url
						path = plugin.settings.folder+'/'+file.name
						media.find('input').val(path.replace('//', '/'));
	...
	//	Read and upload the files
		...
		//	Now post a new XHR request
			...
			{	
				...
				xhr.open('POST', SITE_URL+'/ajax.html');
	...
	//	Reorder the array & make sure the array is well formed
		...
		{
			i = 0;
			$data.find('li:not(.upload)').each(function()
			{
				$li = $(this);
			//	For each media, keep the fields at the same array level				
				...
				//	regexp, catch all [0-9] or [] in the name of the field
					$li.attr('name', $(this).attr('name').replace(/\[(\d?)\]/i, '['+i+']'));
				});
				i++;
			});
		}
